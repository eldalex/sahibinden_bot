...
class Show_results_state(StatesGroup):
    show = State()


class Show_results_dialog():
    def __init__(self, database):
        self.db = database
        self.html_text = Jinja("""""")
        self.image = StaticMedia(url='None')
        self.states_group = Show_results_state()
    ...
    def get_dialog(self):
        ...
        return Dialog(
            Window(
                ...
                Button(Const("Добавить в избранное"), id="add_to_favorite", on_click=self.add_to_favorite, ),
    ...
    async def get_result_data_from_bd(self, **kwargs):
        async with kwargs['state'].proxy() as data:
            ...
            if data.get('result') is None or data.get('result') == []:
                data['result'] = self.db.get_user_result(kwargs['state'].chat, 'all')
                if data['result'] != []:
                    data['current_add'] = 0
                    data['next_add'] = 1
                    data['prev_add'] = 0
    ...
    # Получить id текужего объявления
    async def get_favorite_id(self, manager):
        async with manager.data['state'].proxy() as data:
            return data['result'][data['current_add']][0]

    # Соханить в избранное
    async def add_to_favorite(self, c: CallbackQuery, button: Button, manager: DialogManager):
